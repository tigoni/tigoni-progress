...
## Example: A Simple Payment Listener on Cardano With ,Our, Kafka, and Spring Boot

1. Set Up Oura to Stream Events from Cardano
First, we need to set up Oura to connect to our Cardano node and stream events. Oura can be configured to listen for specific event types, such as `TxSubmission`, which indicates a new transaction has been submitted to the network.
